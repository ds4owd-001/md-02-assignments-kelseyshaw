...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here: The geom_bar() function takes a count of the x variables and plots them, it counts them by default while geom_col() does not.
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe: It changes where the x axis starts and stops and the bar widths inbetween.
